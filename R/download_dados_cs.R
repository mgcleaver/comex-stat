...
#' 
#' @return Um `tibble`.
ler_download_imp <- function(path, tabelas) {
  paises <- tabelas[["paises"]]
  uf <- tabelas[["uf"]]
...
#' 
#' @return Um `tibble`.
ler_download_exp <- function(path, tabelas) {
  paises <- tabelas[["paises"]]
  uf <- tabelas[["uf"]]
...
download_dados_cs <- function(link_download, dirs, tab_aux, schemas) {
  tipo <- stringr::str_extract(link_download, "EXP|IMP") |> stringr::str_to_lower()
  ...
  if (tipo == "imp") {
    path <- stringr::str_subset(dirs, "imp")
    schema_comexstat_imp <- schemas[["imp"]]
    
    x <- ler_download_imp(dir_file_download, tabelas = tab_aux)
    x <- arrow::arrow_table(x)$cast(schema_comexstat_imp)
    
  } else if (tipo == "exp") {
    path <- stringr::str_subset(dirs, "exp")
    schema_comexstat_exp <- schemas[["exp"]]
    
    x <- ler_download_exp(dir_file_download, tabelas = tab_aux)
    x <- arrow::arrow_table(x)$cast(schema_comexstat_exp)
  }
  
  arrow::write_dataset(x, path, partitioning = "co_ano")
  message(glue::glue("Download e escrita de {tipo} {ano_link} ok"))
}